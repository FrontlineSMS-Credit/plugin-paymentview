...
public class PaymentServiceSmsProcessor {
	...
	/**
	 * Stores the indexes of the tags to be used for extracting information from
	 * the source message
	 */
	...
	public PaymentServiceSmsProcessor(String message, String confirmTextTemplate) {
		...
		for (int i = 0; i < templateSMS.length; i++) {
			if (templateSMS[i].equals(PaymentServiceSmsProcessor.TG_AMOUNT))
				tagPositions.put(PaymentServiceSmsProcessor.TG_AMOUNT,
						Integer.toString(i));
			else if (templateSMS[i]
					.equals(PaymentServiceSmsProcessor.TG_BALANCE))
				tagPositions.put(PaymentServiceSmsProcessor.TG_BALANCE,
						Integer.toString(i));
			else if (templateSMS[i]
					.equals(PaymentServiceSmsProcessor.TG_FIRSTNAME))
				tagPositions.put(PaymentServiceSmsProcessor.TG_FIRSTNAME,
						Integer.toString(i));
			else if (templateSMS[i]
					.equals(PaymentServiceSmsProcessor.TG_LASTNAME))
				tagPositions.put(PaymentServiceSmsProcessor.TG_LASTNAME,
						Integer.toString(i));
			else if (templateSMS[i]
					.equals(PaymentServiceSmsProcessor.TG_PHONENUMBER))
				tagPositions.put(PaymentServiceSmsProcessor.TG_PHONENUMBER,
						Integer.toString(i));
			else if (templateSMS[i]
					.equals(PaymentServiceSmsProcessor.TG_TRANSACTION_CODE))
				tagPositions.put(
						PaymentServiceSmsProcessor.TG_TRANSACTION_CODE,
						Integer.toString(i));
		}
	}
	...
	/**
	 * Gets the {@link Client}'s full name by concatenating the first name and
	 * last name from the confirmation text
	 * 
	 * @return
	 */
	...
	/**
	 * Extracts the numerical value from the sourceMessage using the specified
	 * tag
	 * 
	 * @param tag
	 * @return
	...
	private double getNumber(String tag) {
		...
		// Get the string in the source message associated with the retrieved
		// position
		...
		return (numberBuffer.length() > 0) ? Double.parseDouble(numberBuffer
				.toString()) : 0;
	}
