...
public class ClientsTabHandler implements ThinletUiEventHandler {
	...
	public void addClient() {
		ui.add(new EditClientHandler(ui, pluginController, this, true)
				.getDialog());
	}

	public void customizeClientDB() {
		ui.add(new CustomizeClientDBHandler(pluginController, customFieldDao)
				.getDialog());
	}
	...
	public void deleteClient() {
		...
		for (Object selectedClient : selectedClients) {
			Client attachedClient = ui.getAttachedObject(selectedClient,
					Client.class);
			attachedClient.setActive(false);
			clientNameList = clientNameList + " - "
					+ attachedClient.getFullName();
		...
		if (selectedClients.length > 0) {
			ui.infoMessage("You have successfully deleted the following client(s)"
					+ clientNameList + ".");
		}
		this.refresh();
	}
	...
	public void editClient() {
		...
		for (Object selectedClient : selectedClients) {
			...
			ui.add(new EditClientHandler(ui, c, pluginController, this)
					.getDialog());
		}
	}
	...
	public void copyToContacts() throws NumberFormatException,
	...
	private void copyClientsToContacts(List<Client> clients)
			throws NumberFormatException, DuplicateKeyException {
		...
		for (Client c : clients) {
			...
			if (fromMsisdn != null) {
				...
				Contact contact = new Contact(c.getFullName(),
						c.getPhoneNumber(), "", "", "", true);
	...
	public void viewIncomingPaymentByClient() {
		...
		if (selectedItems.length <= 0) {
			if (clientTableHandler.getClientFilter().isEmpty()) {
				...
				selectedClients = this.clientDao
						.getClientsByFilter(clientTableHandler
								.getClientFilter());
			}
		...
		incomingPaymentsDialog = new IncomingPaymentsDialogHandler(ui,
				pluginController, selectedClients).getDialog();
	...
	public void removeFromGroup(Object selectedGroup) throws DuplicateKeyException {
		...
		Object[] selectedClients = this.ui.getSelectedItems(this.clientsTableComponent);
	...
	public void populateGroups(Object popUp, Object list) throws DuplicateKeyException {
		groupDao = pluginController.getUiGeneratorController()
				.getFrontlineController().getGroupDao();
		groupMembershipDao = pluginController
				.getUiGeneratorController().getFrontlineController()
